...
{ 
  ...
  string host = "h", port = "p", file = "f";
  host += (i+'0'); port += (i+'0'); file += (i+'0');
  /* set TCP connection */
  CLIENT *client = &(clients[i-1]);
  client->client_sin.sin_family = AF_INET;
  client->client_sin.sin_addr = *((struct in_addr *)(gethostbyname(parameters[host].c_str())->h_addr));
  client->client_sin.sin_port = htons(atoi(parameters[port].c_str()));
